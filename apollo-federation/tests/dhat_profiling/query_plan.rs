...
fn valid_query_plan() {
    ...
    // Actual number: 879_401.
    // (Increased from 752_333 due to SemanticEdgeId/SemanticNodeId index HashMaps on QueryGraph.)
    const MAX_BYTES_QUERY_PLANNER: usize = 923_371; // ~901 KiB

    // Total number of allocations with a 5% buffer.
    // Actual number: 16_262.
    const MAX_ALLOCATIONS_QUERY_PLANNER: u64 = 17_075;

    // Number of bytes when the heap size reached its global maximum with a 5% buffer.
    // Actual number: 872_551.
    //
    // Planning adds ~0 bytes to heap max beyond planner construction because the semantic
    // indexes are built at QueryPlanner::new time, not during planning.
    const MAX_BYTES_QUERY_PLAN: usize = 1_043_141; // ~1018 KiB

    // Total number of allocations with a 5% buffer.
    // Actual number: 23_719.
    //
    // Planning adds allocations for traversal, condition resolution, path trees, FDG.
